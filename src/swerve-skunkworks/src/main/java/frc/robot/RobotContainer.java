...
import frc.robot.commands.ToggleVisionCorrectionCommand;
...
import frc.robot.commands.FindAndApproachNearestAprilTagCommand;
...
public class RobotContainer
{
  ...
  private final FindAndApproachNearestAprilTagCommand findAprilTagCommand;
  private final SwerveSubsystem       drivebase  = new SwerveSubsystem(new File(Filesystem.getDeployDirectory(),
                                                                                "swerve/neo"));
  private final OdometryPoseEstimator poseEstimator = new OdometryPoseEstimator(drivebase);
  ...
  {
    // Initialize the find AprilTag command
    findAprilTagCommand = new FindAndApproachNearestAprilTagCommand(drivebase, drivebase.getVision());
    ...
    SmartDashboard.putBoolean("Vision/CorrectionEnabled", true);
    SmartDashboard.putBoolean("Vision/HasTarget", false);
    SmartDashboard.putBoolean("Vision/Active", false);
    SmartDashboard.putNumber("Vision/LatencyMs", 0);
  }
...
//                              );
  ...
  private void configureButtonBindings() {
    // Find and approach the nearest AprilTag when A button is pressed
    driverXbox.a().toggleOnTrue(findAprilTagCommand);
    
    // Initialize the waypoint command
    waypointCommand = new WaypointCommand(
        drivebase,
        poseEstimator,
        driverXbox.rightBumper(),  // Right bumper to set waypoint
        driverXbox.leftBumper()    // Left bumper to return to waypoint
    );
    
    // Schedule the command to run in the background
    waypointCommand.schedule();
  }
  ...
  /**
   * Updates the pose estimator. Should be called in robotPeriodic.
   */
